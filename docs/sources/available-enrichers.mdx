...
**org_to_pappers**: Enrich organizations with legal, financial, and beneficial ownership (bénéficiaires effectifs) data from Pappers (France only).
Tools/APIs: [Pappers API](https://www.pappers.fr/api)
...
**username_to_instagram**: Get an Instagram profile (bio, follower counts, public contact info) for a username, via Osintgram/HikerAPI.
Tools/APIs: [HikerAPI](https://hikerapi.com/) (used by [Osintgram](https://github.com/Datalux/Osintgram))

### Website
**website_to_crawler**: Crawl a website to extract emails and phone numbers.
Tools/APIs: ReconCrawlTool (`reconcrawl`)

**website_to_torbot**: Crawl a `.onion` (or clearnet) page over Tor, discovering outbound links and public contact info, one level deep.
Tools/Pivots: Tor SOCKS5 proxy, in the spirit of [TorBot](https://github.com/DedSecInside/TorBot)
...
- API-keyed enrichers read keys from params or environment (e.g., `HIBP_API_KEY`, `ETHERSCAN_API_KEY`, `WHOXY_API_KEY`, `WHOISXML_API_KEY`, `DEHASHED_API_KEY`, `SCAMALYTICS_API_KEY`, `VERIPHONE_API_KEY`, `C99_API_KEY`, `PAPPERS_API_KEY`, `HIKERAPI_TOKEN`).
- `website_to_torbot` needs a running Tor daemon (SOCKS5 proxy, default `127.0.0.1:9050`) to reach `.onion` addresses; it can also be pointed at clearnet pages with `use_tor` disabled.
